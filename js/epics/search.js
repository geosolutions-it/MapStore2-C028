...
const registerSearchServiceEpic = action$ => action$.ofType(LOCAL_CONFIG_LOADED).switchMap(() => {
    ...
    const bzComuniCatastali = (searchText, {comuni = [
        ...
        const match = comuni.filter( ({name =""}) => name.toLowerCase().indexOf(searchText.toLowerCase()) >= 0);
        const results = match.map( createEntry("Particella Fondiaria", " partfond"))
        .concat(
            match.map(createEntry("Particella Edificabile", "partedif"))
        );
